  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
      <div className="mb-10 flex items-end justify-between">
        ...
        <Link
          href="/collections"
          className="text-xs font-semibold uppercase tracking-widest text-svate-gold transition-opacity hover:opacity-80 sm:text-sm"
        >
          View All
        </Link>
      </div>
        ...
        {products.map((product) => (
          <article key={product.name} className="group flex flex-col">
            <Link href="/collections" className="block">
              <div className="relative mb-4 aspect-[3/4] overflow-hidden rounded-2xl bg-svate-beige/40">
                <Image
                  src={product.image}
                  alt={product.name}
                  fill
            ...
            <div className="mt-3 flex gap-2" aria-label={`${product.name} shades`}>
              {product.shades.map((shade) => (
                <span
                  key={shade}
                  className="h-4 w-4 rounded-full border border-svate-beige ring-1 ring-transparent transition-shadow hover:ring-svate-gold"
